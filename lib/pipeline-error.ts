 ...
 *   - `userMsg`     — user-facing toast text (localized via normalizeError)
 ...
 */
export function normalizeError(err: unknown, stage: PipelineStage = 'unknown', locale: Locale = 'en'): PipelineError {
  ...
  if (/1026|sensitive/i.test(msg)) {
    return createError('SENSITIVE_INPUT', apiT(locale, 'sensitive'), {
      stage, cause: raw, retryable: true,
    });
  }
  if (/timeout|timed out|ETIMEDOUT/i.test(msg)) {
    return createError('TIMEOUT', apiT(locale, 'timeout'), {
      stage, cause: raw, retryable: true,
    });
  }
  if (/fetch failed|ECONNRESET|ENOTFOUND|network/i.test(msg)) {
    return createError('NETWORK', apiT(locale, 'network'), {
      stage, cause: raw, retryable: true,
    });
  }
  if (/no URL in response|INVALID_RESPONSE|invalid URL/i.test(msg)) {
    return createError('INVALID_RESPONSE', apiT(locale, 'invalidResponse'), {
      stage, cause: raw, retryable: true,
    });
  }
  if (/not available|not configured|missing key/i.test(msg)) {
    return createError('ENGINE_UNAVAILABLE', apiT(locale, 'engineUnavailable'), {
      stage, cause: raw, retryable: false,
    });
  }
  if (/ffmpeg/i.test(msg)) {
    return createError('FFMPEG', apiT(locale, 'ffmpeg'), {
      stage, cause: raw, retryable: true,
    });
  }

  return createError('UNKNOWN', msg || apiT(locale, 'unknownError'), { stage, cause: raw, retryable: true });
}
 ...
 */
export function toSsePayload(err: unknown, locale: Locale = 'en') {
  const pe = err instanceof PipelineError ? err : normalizeError(err, 'unknown', locale);
  return {
    code: pe.code,
    userMsg: pe.userMsg,
