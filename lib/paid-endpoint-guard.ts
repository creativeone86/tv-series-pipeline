 ...
 *
 *   const g = await guardPaidEndpoint(request, { pendingCostEur: 0.23 });
 *   if (!g.ok) return g.response;
 *   // g.userId 可用于记账
 *
 * 注意 `pendingCostEur` 是**本次操作的预估成本**,用于「这一笔会不会把人顶过上限」的前瞻判断;
...
export async function guardPaidEndpoint(
  ...
  const { allow, guard } = await assertBudget({ userId: u.userId, pendingCostEur: opts.pendingCostEur, locale });
  if (!allow) {
